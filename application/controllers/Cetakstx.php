...
class Cetakstx extends CI_Controller
{
  ...
  function packinglist(){
    ...
    if($cek->row("siap_jual") == "n" OR $wicode==29){
        $__jumlah_valid = $cek->row("jumlah_roll");
    } else {
        $__jumlah_valid = $this->db->query("SELECT * FROM new_tb_isi WHERE kd='$token' AND validasi='valid'")->num_rows();
    }
    // if($__jumlah_roll != $__jumlah_valid){
    //     echo "<div style='width:100%;height:100vh;display:flex;justify-content:center;align-items:center;flex-direction:column;'>";
    //     echo "<span style='color:red;'>Belum valid</span>";
    //     echo "<span>Jumlah Roll : ".$__jumlah_roll."</span>";
    //     echo "<span>Jumlah Validasi : ".$__jumlah_valid."</span>";
    //     echo "</div>";
    // } else {
    if($cek->num_rows() == 1){
        $kode_roll = array();
  ...
  //   );
    ...
    if($cek->num_rows() == 1){
        ...
        $ex = explode('-', $row['tanggal_dibuat']);
              ...
              $pdf->SetFont('Arial','',10);
              if($jml_pkt<121){ $pdf->Text(10,256, 'PT. RINDANG JATI SPINNING'); $pdf->Text(90,256, 'SUPIR'); $pdf->Text(165,256, 'PEMBELI'); }
              $pdf->Ln(25);
            ...
            $pdf->Ln(18);
              ...
              $pdf->SetFont('Arial','',10);
              if($jml_pkt<261){ $pdf->Text(10,256, 'PT. RINDANG JATI SPINNING'); $pdf->Text(90,256, 'SUPIR'); $pdf->Text(165,256, 'PEMBELI'); }
              $pdf->Ln(35);
