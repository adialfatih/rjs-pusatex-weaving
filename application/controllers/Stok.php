...
class Stok extends CI_Controller
{
  ...
  function showsj(){
    ...
    function cronnotappn(){
        $ppn = $this->db->query("SELECT * FROM a_nota WHERE pajak_ppn > 0 AND cron_job='null'")->result();
        foreach($ppn as $val){
            $id_nota = $val->id_nota;
            $pj = $val->total_panjang;
            $harga_asli = $val->harga_satuan;
            $pajak = $val->pajak_ppn;
            $pembagi = 1 + ($pajak/100);
            $harga_setelah_pajak = $harga_asli/$pembagi;
            //$harga_setelah_pajak = round($harga_setelah_pajak);
            $total_harga_asli = $pj * $harga_setelah_pajak;
            $total_harga_asli_plus_ppn = $pembagi * $total_harga_asli;

            echo "Total Panjang = $pj <br>";
            echo "Harga Asli = $harga_asli <br>";
            echo "Pajak = $pajak <br>";
            echo "Harga Setelah Pajak = $harga_setelah_pajak <br>";
            echo "Total Harga Asli : $pj * $harga_setelah_pajak = $total_harga_asli <br>";
            echo "Total Harga +PPN :  $total_harga_asli_plus_ppn <br>";
            echo "<hr>";
            $updates = [
                'harga_satuan'  => $harga_setelah_pajak,
                'total_harga'   => round($total_harga_asli_plus_ppn),
                'total_harga_asli' => round($total_harga_asli,2),
                'cron_job'      => 'finish'
            ];
            $this->data_model->updatedata('id_nota',$id_nota,'a_nota',$updates);
            
        }
        echo "Sedang mengupdate nota...";
        echo "<script>
                window.onload = function() {
                    setTimeout(function() {
                        window.close();
                    }, 1500); // tunggu 2 detik sebelum menutup (opsional)
                };
            </script>";
    }
