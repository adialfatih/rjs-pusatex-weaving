...
<body>
    ...
    <?php
        $ig = $this->db->query("SELECT kode_roll,loc_now FROM data_ig WHERE loc_now='Samatex' AND kode_roll NOT IN (SELECT kode_roll FROM data_fol) AND kode_roll NOT IN (SELECT kode_roll FROM data_if)");
        $ar_ig = array();
