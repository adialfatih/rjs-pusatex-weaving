...
<head>
    ...
    <title>Produksi Folding Grey</title>
...
<body>
    ...
    $ts = $this->db->query("SELECT id_fol,kode_roll,jns_fold,jam_input FROM `data_fol` WHERE kode_roll NOT LIKE '%JP%' AND jns_fold = 'Grey' ORDER BY id_fol DESC LIMIT 1")->row("jam_input");
    $xp = explode(' ', $ts);
    $jam = explode(':', $xp[1]);
    $tg = explode('-', $xp[0]);
