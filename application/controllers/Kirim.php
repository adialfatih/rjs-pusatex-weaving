...
class Kirim extends CI_Controller
{
  ...
  //             $notelp = $this->data_model->filter($this->input->post('notelp'));
  //             $almt = $this->data_model->filter($this->input->post('almt'));
  //             $ceknum = $this->db->query("SELECT * FROM surat_jalan WHERE departement='$dep' ORDER BY id_sj DESC LIMIT 1");
  ...
  function kirim_paket2(){
      ...
      $qry = "SELECT * FROM surat_jalan WHERE tujuan_kirim='cus' AND DATE_FORMAT(tgl_kirim, '%Y-%m') = '$bulan_sekarang' ORDER BY id_sj DESC LIMIT 1";
      $_cek = $this->db->query($qry);
      if($_cek->num_rows() == 1){
          $no_urt = $_cek->row("no_urut");
          $no_urut = intval($no_urt) + 1;
      } else {
          $no_urut = 1;
      }
      $jual = $this->data_model->clean($this->input->post('thisjnspaket'));
      $tipeSend = $this->input->post('tipeSend');
      ...
      if($jual=="y"){
        if($cekcus->num_rows() == 1){
            ...
            $this->session->set_flashdata('announce', 'Pengiriman ke Customer berhasil');
      ...
      if($tipeSend=="cus"){
          if($cekcus->num_rows() == 1){
              ...
              for ($i=0; $i < count($pkg); $i++) { 
                $this->data_model->updatedata('kd',$pkg[$i],'new_tb_packinglist',['kepada'=>$namacus,'no_sj'=>$surat_jalan]);
                $dtpkg = $this->data_model->get_byid('new_tb_packinglist', ['kd'=>$pkg[$i]]);
                $this->data_model->updatedata('loc_now',$pkg[$i],'data_ig',['loc_now'=>$namacus]);
              }
              $this->session->set_flashdata('announce', 'Pengiriman ke Customer berhasil');
              redirect(base_url('packing-list'));
          } else {
          ...
          $this->data_model->saved('surat_jalan',$dt_list);
          for ($i=0; $i < count($pkg); $i++) { 
            $tes1 = $this->data_model->get_byid('new_tb_packinglist', ['kd'=>$pkg[$i]])->row("customer");
            if($tes1=="Finish"){
              $tes23 = "Finish";
            } else {
              $tes23 = "Grey";
            }
            $cekig = $this->data_model->get_byid('data_ig', ['loc_now'=>$pkg[$i]]);
              foreach($cekig->result() as $ig){
                  $_kode_roll = $ig->kode_roll;
                  echo $_kode_roll."-";
                  $_ukuran = $ig->ukuran_ori;
                  $_mc = $ig->no_mesin;
                  $_kons = $ig->konstruksi;
                  $lit = $this->data_model->get_byid('kiriman_pusatex', ['kode_roll'=>$_kode_roll])->num_rows();
                  if($lit==0){
                      $this->data_model->saved('kiriman_pusatex',[
                        'kode_roll' => $_kode_roll,
                        'mc' => $_mc,
                        'ukuran' => $_ukuran,
                        'konstruksi' => $_kons,
                        'tanggal' => $tgl_kirim,
              ...
              $this->data_model->updatedata('kd',$pkg[$i],'new_tb_packinglist',['kepada'=>'Pusatex','no_sj'=>$surat_jalan]);
              $this->data_model->updatedata('loc_now',$pkg[$i],'data_ig',['loc_now'=>'Pusatex']);
              
          }
          $this->session->set_flashdata('announce', 'Pengiriman ke Pusatex berhasil');
          redirect(base_url('packing-list'));
          //echo "owek";
      }
    }
