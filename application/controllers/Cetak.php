...
class Cetak extends CI_Controller
{
  ...
  function processNama($nama_cus) {
    // Memeriksa apakah nama dimulai dengan "PS", "KM", atau "PH" diikuti spasi
    if (preg_match('/^(PS|KM|PH)\s/', $nama_cus)) {
        // Jika nama dimulai dengan kode tersebut, menghapus kode tersebut
        $nama_cus = preg_replace('/^(PS|KM|PH)\s/', '', $nama_cus);
    }
    // Mengembalikan nama yang sudah diproses
    return $nama_cus;
 }
 function processNama2($nama_cus) {
    $prefixes = [
        'PS' => 'PUTRI SEJATI',
        'PH' => 'PUTRI HANAN',
        'KM' => 'KEMBANG MENAWAN'
    ...
    // Loop untuk mengecek dan mengganti jika cocok
    foreach ($prefixes as $prefix => $replacement) {
        if (stripos($nama_cus, $prefix . ' ') === 0) {
            // Menghapus prefix dari awal string dan mengganti dengan nama lengkap
            $nama_baru = trim(substr($nama_cus, strlen($prefix)));
            return $replacement . ' - ' . $nama_baru;
        }
    }

    // Jika tidak ada prefix yang cocok, kembalikan nama asli
    return $nama_cus;
 }
  ...
  function suratjalan(){
      ...
      if($cek->num_rows() == 1){
        ...
        $nama_cus2 = $this->processNama($nama_cus);
  ...
  function nota(){
    ...
    $dua_karakter_terakhir = substr($tg[0], -2);
    $_thisNoNota = $tg[1]."".$dua_karakter_terakhir."".$no_urut_sj;
    ...
    foreach($pkg->result() as $val):
      $roll_total = $val->jml_roll;
      $panjang_total = $val->total_panjang;
      $pres_sb = $val->pres_sb;
      $pres_ecer = $val->pres_ecer;
      $pres_notad = $val->pres_notad;
      $jns[]=$val->konstruksi;
      $roll[]=$roll_total;
      //---1 
      $roll_1 = (intval($roll_total) * intval($pres_sb)) / 100;
      $roll_2 = (intval($roll_total) * intval($pres_ecer)) / 100;
      $roll_3 = (intval($roll_total) * intval($pres_notad)) / 100;
      $roll1[] = round($roll_1,2);
      $roll2[] = round($roll_2,2);
      $roll3[] = round($roll_3,2);
      //---1
      $pjg[]=$panjang_total;
      $pjg_1 = (floatval($panjang_total) * intval($pres_sb)) / 100;
      $pjg_2 = (floatval($panjang_total) * intval($pres_ecer)) / 100;
      $pjg_3 = (floatval($panjang_total) * intval($pres_notad)) / 100;
      $pjg1[] = round($pjg_1,2);
      $pjg2[] = round($pjg_2,2);
      $pjg3[] = round($pjg_3,2);
      $hrg[]=$val->harga_satuan;
      $total[]=$val->total_harga_asli;
      $total23[]=$val->total_harga;
      //$totalHarga_dgppn = $totalHarga_dgppn + $val->total_harga;
      $nkode = $val->kd;
      $ppn = $val->pajak_ppn;
      $ppnnota[] = $val->pajak_ppn;
      if($ppn>0){
          $_thisppn = (intval($ppn) * $val->total_harga_asli) / 100;
          $_nilaiPPN += $_thisppn;
      }
      $satuans[] = $this->db->query("SELECT * FROM new_tb_isi WHERE kd='$nkode' LIMIT 1")->row("satuan");
    endforeach;
    //$totalHarga2 = $totalHarga_dgppn + $_nilaiPPN;
    $sumTotal = array_sum($total23);
    $pdf = new FPDF('l','mm',array(240,140));
              // membuat halaman baru
              for ($ad=0; $ad <4 ; $ad++) { 
                if($ad==0){ $_thisTipeKode = ""; } 
                if($ad==1){ $_thisTipeKode = "ENP"; } 
                if($ad==2){ $_thisTipeKode = "EP"; } 
                if($ad==3){ $_thisTipeKode = "D"; } 
              $pdf->AddPage();
              // setting jenis font yang akan digunakan
              ...
              $pdf->Cell(0, 8, '', 0, 0, 'R' );
              $pdf->Text(160,13, 'Kepada YTH :');
              if($namacus=="PUTRI DIANA56"){
                  $namacus = "PUTRI DIANA";
              } elseif($namacus=="SALIM56"){
                  $namacus = "Helmy PB";
              } else {
                  $namacus = $this->processNama2($namacus);
                  
              }
              $pdf->Text(160,18, ''.$namacus.'');
              $pdf->Text(160,23, $almtcus);
              $pdf->Ln(8);
              $pdf->Ln(8);
              $pdf->Cell(45, 5, 'No Nota', 0, 0, 'L' );
              $pdf->Cell(5, 5, ':', 0, 0, 'L' );
              $pdf->Cell(100, 5, ''.$_thisNoNota.''.$_thisTipeKode.'', 0, 0, 'L' );
              $pdf->Ln(5);
              ...
              $pdf->Ln($jarakbaris);
              $baris_bawah = 53;
              $_thisAllTotalHarga=0;
              $_thisTotalPPN=0;
              for ($i=0; $i <$io ; $i++) { 
              $no=$i+1;
              $pdf->Cell(20, 5, $no, 0, 0, 'C' );
              $pdf->Cell(40, 5, $jns[$i], 0, 0, 'C' );
              if($ad==0){ $_thisRoll = $roll[$i]; $_thisPjg = $pjg[$i]; } 
              if($ad==1){ $_thisRoll = $roll1[$i]; $_thisPjg = $pjg1[$i]; } 
              if($ad==2){ $_thisRoll = $roll2[$i]; $_thisPjg = $pjg2[$i]; } 
              if($ad==3){ $_thisRoll = $roll3[$i]; $_thisPjg = $pjg3[$i]; } 
              $pdf->Cell(20, 5, $_thisRoll, 0, 0, 'C' );
              $stst = $satuans[$i]=="Meter" ? 'Meter':'Yard';
                if(fmod($_thisPjg, 1) !== 0.00){
                  $crt1 = number_format($_thisPjg,2,',','.')." ";
                } else {
                  $crt1 = number_format($_thisPjg,0,',','.')." ";
                }
              $pdf->Cell(50, 5, ''.$crt1.''.$stst, 0, 0, 'C' );
              $_thisTotalNilai = $_thisPjg * $hrg[$i];
              $_thisAllTotalHarga += $_thisTotalNilai;
              $_nPPN = ($ppnnota[$i] * $_thisTotalNilai) / 100;
              $_hnPPN = $_thisTotalNilai + $_nPPN;
              $_thisTotalPPN += $_nPPN;
              ...
              $pdf->Cell(35, 5, $crt2, 0, 0, 'C' );
                if(floor($_thisTotalNilai) == $_thisTotalNilai){
                  $crt3 = number_format($_thisTotalNilai,0,',','.')." ";
                } else {
                  $crt3 = number_format($_thisTotalNilai,2,',','.')." ";
                }
              ...
              if($_thisTotalPPN>0){
                ...
                $pdf->Cell(20, 5, '', 0, 0, 'C' );
                $pdf->Cell(50, 5, '', 0, 0, 'C' );
                $pdf->Cell(35, 5, 'PPn', 0, 0, 'R' );
                //$_thiPPN = ($ppn);
                  if(fmod($_thisTotalPPN, 1) !== 0.00){
                    $_nilaiPPN2 = number_format($_thisTotalPPN,2,',','.')." ";
                  } else {
                    $_nilaiPPN2 = number_format($_thisTotalPPN,0,',','.')." ";
                  }
                $pdf->Cell(55, 5, 'Rp. '.$_nilaiPPN2, 0, 0, 'R' );
              }
              if($io<4){
                $pdf->Ln(5);
              } else {
              ...
              $_xxy = $_thisAllTotalHarga + $_thisTotalPPN;
                if(fmod($_xxy, 1) !== 0.00){
                  $crt67 = number_format($_xxy,2,',','.')." ";
                } else {
                  $crt67 = number_format($_xxy,0,',','.')." ";
                }
              $pdf->Cell(55, 5, 'Rp. '.$crt67, 0, 0, 'R' );
