						...
						<div class="pb-20">
							<table class="data-table table stripe hover nowrap">
								...
								foreach($kons->result() as $val){
									...
									$fg = $this->db->query("SELECT SUM(ukuran) AS jm FROM data_fol WHERE konstruksi='$k' AND jns_fold='Grey' AND posisi='Pusatex' ")->row("jm");
									$fg_2 = $this->db->query("SELECT SUM(ukuran) AS jm FROM data_fol WHERE konstruksi='$k' AND jns_fold='Grey' AND posisi LIKE '%PKT%' ")->row("jm");
									$ff = $this->db->query("SELECT SUM(ukuran) AS jm FROM data_fol WHERE konstruksi='$k' AND jns_fold='Finish' AND posisi='Pusatex'")->row("jm");
