...
class Inputt extends CI_Controller
{
  ...
  function produksikirim(){
        ...
        $query = $this->db->query("SELECT * FROM surat_jalan WHERE dep_asal='$dep' AND no_sj NOT LIKE '%SLD%' ORDER BY id_sj DESC LIMIT 1000");
        $data = array(
  ...
  function promesin(){ 
        ...
        $datanew = implode(',', $data);
            $data = array(
                ...
                'dataauto' => $datanew,
                'dep_user' => $dep,
                'dt_table' => $this->db->query("SELECT * FROM dt_produksi_mesin WHERE lokasi='$dep' ORDER BY id_produksi_mc DESC LIMIT 1000")
            );
            $this->load->view('part/main_head', $data);
            $this->load->view('part/left_sidebar', $data);
  ...
  function penjualan(){ 
        ...
        $datanew = "tes";
            $data = array(
                ...
                'dataauto' => $datanew,
                'dep_user' => $dep,
                'dtpnj' => $this->db->query("SELECT * FROM `a_nota` WHERE YEAR(tmstmp) = '$yr' ORDER BY tgl_nota DESC")
            );
            $this->load->view('part/main_head', $data);
            $this->load->view('part/left_sidebar', $data);
            $this->load->view('page/input_penjualan', $data);
